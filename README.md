...
- **子 Agent 协作** —— 隔离子任务图（`mode="subtask"`，无风险/确认节点、防递归），线程池并行，主消息只留折叠摘要；父任务 `stop()` 会传播到正在跑的子任务，不再只能等 `subagent_timeout_sec` 兜底（[#60](https://github.com/renjianguojinqianfan/langgraph-agent/issues/60)）
