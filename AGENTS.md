...
## 1. 快照
基于 **LangGraph 1.2.x（StateGraph）** 的自主任务 Agent 平台，前后端一体：自然语言下发任务 → Agent 自主规划（planner→executor→tool→reflect 循环）→ 调用工具完成多步任务 → SSE 实时可视化。经 v1 + P0（对齐 HelloAgents/DeepAgent）+ P1（六项能力）+ P2（MCP/Git）+ P3（断点续跑，Issue #4）+ langgraph 0.2→1.2.x 原地迁移（Issue #7）多轮迭代，**362 个离线测试全绿** + 真实 LLM（qwen3.6-plus）端到端与断点续跑双场景验证通过。
...
- **依赖矩阵**：`langgraph>=1.2,<2`（1.2.x LTS 线）+ `langgraph-checkpoint-sqlite==3.1.1`（钉死：存储格式与 serde 直接决定 resume 契约，抬版本需人评估放行，Dependabot 自动抬版勿直接合）。`langchain*` 不得重新加回显式钉版（全仓 0 import，且 `langchain-core<0.3` 与 langgraph 1.x 直接冲突）；uvicorn/starlette 区间不动。
- **旧快照作废 + 启动检测**（Issue #7）：迁移前写的 checkpoint 库一律作废。上游没留版本痕迹（2.0.11 与 3.1.1 建表 SQL 逐字符相同），所以标记由本仓库自己打：挂载成功即 `PRAGMA user_version = CHECKPOINT_FORMAT_VERSION`；有行且未打标 → ERROR 告警 + **拒绝挂载**（`_checkpointer` 留 None，服务照常起、新任务照常跑，resume 落回「no checkpoint → 409」）。不做自动迁移、不改被拒文件一个字节。判定逻辑在 `task_manager._inspect_checkpoint_store`，实测依据见 `docs/migration-langgraph-1x.md` §2。
...
| `backend/tests/` | **40 文件 / 362 用例**（含 test_qa_* 独立补充；test_checkpointer/test_resume/test_orphan_reconcile 为 P3）|
...
## 5. 完成定义
- 后端 `.venv311 python -m pytest backend/tests/ -q` 全绿（362）；改前端时 `npx tsc --noEmit` 0 错误。
